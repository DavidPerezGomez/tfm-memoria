...
\section{Efficiency in RL}

In chapter \ref{chapter:introduction} we mentioned how research often focuses on the accuracy of the final models. However, that doesn't mean that no effort is dedicated to improving training efficiency. Reinforcement learning algorithms are notoriously sample-inefficient and slow to converge \cite{Botvinick:2019}, making them less than ideal for real-time tasks \cite{Bianchi:2004}.

One strategy that has been studied with the goal of reducing the learning time is the introduction of a heuristic function \cite{Cheng:2021}. The heuristic $h:\mathcal{S}\to\mathbb{R}$ represents a guess of the optimal value function, usually derived from previous experience with the environment, and it is used to guide the agent's learning process. At the beginning of the training, the discount factor for future rewards is increased more than usual, shortening the agent's horizon and making it prefer short-term rewards, which are influenced by the heuristic. As the training progresses and the agent gains more knowledge, the discount is steadily reduced to normal levels, while the effect of the heuristic is also toned down. This algorithm was shown to improve learning efficiency \cite{Cheng:2021}. However, it comes wth the limitation that it requires the existence of a heuristic functions, and its effectiveness is tied to how good said heuristic is, since a bad heuristic can have the opposite effect of slowing down training.

% [Heuristic-Guided Reinforcement Learning](https://proceedings.neurips.cc/paper_files/paper/2021/file/70d31b87bd021441e5e6bf23eb84a306-Paper.pdf) QL

% [Heuristically Accelerated Q–Learning: a new approach to speed up Reinforcement Learning](https://d1wqtxts1xzle7.cloudfront.net/76561779/BianchiRibeiroCosta04-LNAI-libre.pdf?1639682088=&response-content-disposition=inline\%3B+filename\%3DHeuristically_Accelerated_Q_Learning_a_n.pdf&Expires=1730472021&Signature=H9adm4S3U55uv421-gPeE3Pbd8J49MlYWasFxZZ7~KFfDdDIbM7eARRyUpJeHAeS0P1oq~me~T5aHOsNGocCmazBEMjhJVeVG6QGIh1Og~lqcBNzv6bckLM174Iap72c34TeDCORQ-4tJoC-Lx54QajGDkWM8jireXeajeS0Su~1qNhuGl8s69SZ0Fo0DHVmD34wEmgdD0A7iXWobuPklJdl5CivdIyg~bUftnwY5sJOZ6HFPmHhER3DRtUZW96UflwAuedas~wsiWDe5tFymKKXWTcVaRwsnIsHUFojvZH4uK~CV1hh2jq8D9M3fczn1GRF9YakYhfgSNjQi3vtXw__&Key-Pair-Id=APKAJLOHF5GGSLRBV4ZA) QL

% Deep reinforcement learning is a powerful tool

% [Reinforcement Learning, Fast and Slow](https://www.cell.com/trends/cognitie-sciences/fulltext/S1364-6613(19)30061-0) DRL

% [Why Does Hierarchy (Sometimes) Work So Well in Reinforcement Learning?](https://arxiv.org/abs/1909.10618) HRL
