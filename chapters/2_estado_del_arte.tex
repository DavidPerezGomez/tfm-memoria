...
Reinforcement learning is a form of machine learning that can be described as \say{a way of programming agents by reward and punishment without needing to specify how the task is to be achieved} \cite{Kaelbling:1996}.
...
\begin{description}
    \item[$\mathcal{S}$] is the finite set of all possible states in the environment ($S_t \in \mathcal{S}$). These states follow the Markov property, which means that for any state, the future states are independent of the past states \cite[unk]. Its elements can be of one of two forms, depending on whether or not the reward for the transition is deterministic.
    \begin{equation}
    ...
    \item[$\mathcal{P}$] is the state transition matrix, which contains the probabilities for all the transition from a state $S_t=s$ to the next state $S_{t+1}=s^\prime$.
    \begin{equation}
...
% \subsection{Hierarchical deep reinforcement learning}